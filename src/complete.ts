...
export function expressionLanguageCompletion(context: CompletionContext): CompletionResult | null {
  ...
  if ((prevNode.parent?.type.is(PropertyAccess) || prevNode.parent?.type.is(MethodAccess)) && [PropertyAccess, MethodAccess, ArrayAccess, Variable, Call, Application].includes(prevNode.parent.firstChild?.type.id ?? -1)) {
  ...
  if (
    ...
    || [Expression, UnaryExpression, BinaryExpression, TernaryExpression].includes(prevNode.parent?.type.id ?? -1) && prevNode.type.isError
    || [Variable, Function].includes(prevNode.parent?.type.id ?? -1) && prevNode.type.isError
  ) {
